...
<div id="content"><!--Start of content for delete
	page-->
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	// Make the query
		...
		if (mysqli_affected_rows($dbcon ) == 1) { // If there was no problem
			...
			echo '<p class="error">The record could not be
			deleted.<br>Probably
			because it does not exist or due to a system
			error.</p>'; // Display error message
			echo '<p>' . mysqli_error($dbcon ) . '<br/>Query:
			' . $q . '</p>';
			// Debugging message
		}
...
// Get the member's data
		$row = mysqli_fetch_array ($result, MYSQLI_NUM);// Display the name of the member being deleted
	echo "<h3>Are you sure you want to permanently
	delete $row[0]?</h3>";
// Display the delete page
echo '<form action="delete_record.php"
	method="post">
#4
