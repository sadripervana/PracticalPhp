...
<body>
	<div id="container">
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			//remove spaces from begginging and end of the email address and validate it
			...
			// Trim the country
			$county = trim($_POST['country']);
			// Strip HTML and apply escaping 
			$stripped = mysqli_real_escape_string($dbcon, strip_tags($county));
			// Get string lengths
			$strlen = mb_strlen($stripped, 'utf8');
			// Check stripped string
			if($strlen < 1) {
				$errors[] = 'You forgot to enter your coundtry';
			} else {
				$county = $stripped;
			}
			// Trim the post code
			...
			if(empty($errors)){ // If everything's OK make the query
